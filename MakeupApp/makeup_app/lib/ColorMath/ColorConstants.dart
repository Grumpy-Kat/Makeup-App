// Credit: https://github.com/gtaylor/python-colormath/

//Lab Calculations
const double CIE_E = 216.0 / 24389.0;
const double CIE_2000 = 6103515625;

//Observer Function and Illuminant Data
const Map<String, Map<String, List<double>>> ILLUMINANTS = {
  //2 Degree Functions
  '2': {
    'a': [1.09850, 1.00000, 0.35585],
    'b': [0.99072, 1.00000, 0.85223],
    'c': [0.98074, 1.00000, 1.18232],
    'd50': [0.96422, 1.00000, 0.82521],
    'd55': [0.95682, 1.00000, 0.92149],
    'd65': [0.95047, 1.00000, 1.08883],
    'd75': [0.94972, 1.00000, 1.22638],
    'e': [1.00000, 1.00000, 1.00000],
    'f2': [0.99186, 1.00000, 0.67393],
    'f7': [0.95041, 1.00000, 1.08747],
    'f11': [1.00962, 1.00000, 0.64350],
  },
  //10 Degree Functions
  '10': {
    'd50': [0.9672, 1.000, 0.8143],
    'd55': [0.958, 1.000, 0.9093],
    'd65': [0.9481, 1.000, 1.073],
    'd75': [0.94416, 1.000, 1.2064],
  }
};

//Observer Function
const List<String> OBSERVERS = [ '2', '10' ];

//Chromatic Adaption Matrices
const Map<String, List<List<double>>> ADAPTATION = {
  'xyz_scaling': [
    [1.00000, 0.00000, 0.00000],
    [0.00000, 1.00000, 0.00000],
    [0.00000, 0.00000, 1.00000],
  ],
  'bradford': [
    [0.89510, 0.26640, -0.1614],
    [-0.7502, 1.71350, 0.03670],
    [0.03890, -0.0685, 1.02960],
  ],
  'von_kries': [
    [0.400240, 0.70760, -0.0808],
    [-0.22630, 1.16532, 0.04570],
    [0.000000, 0.00000, 0.91822],
  ]
};